  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-12">
      <div className="flex items-center justify-between mb-8">
        ...
        <Link href="/admin/new" className="px-5 py-2.5 bg-amber-500 text-white rounded-full font-semibold hover:bg-amber-600 transition-colors">+ New Post</Link>
      </div>
      {posts.length === 0 ? (
          ...
          {posts.map(post => (
            <div key={post.id} className="bg-white rounded-xl border border-stone-100 p-4 flex items-center gap-4">
              ...
              <div className="flex items-center gap-2">
                <Link href={`/blog/${post.slug}`} className="text-xs px-3 py-1.5 border border-stone-200 rounded-lg hover:bg-stone-50 transition-colors">View</Link>
                <Link href={`/admin/edit/${post.id}`} className="text-xs px-3 py-1.5 bg-stone-100 hover:bg-stone-200 rounded-lg transition-colors">Edit</Link>
                <button onClick={() => handleDelete(post.id)} className="text-xs px-3 py-1.5 bg-red-50 text-red-600 hover:bg-red-100 rounded-lg transition-colors">Delete</button>
              </div>
            </div>
          ))}
        </div>
