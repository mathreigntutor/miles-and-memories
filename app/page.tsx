    ...
    <div>
        ...
        {featured ? (
          <>
            ...
            <div className="relative z-10 max-w-6xl mx-auto px-4 pb-12 w-full">
              <span className="inline-block bg-amber-400 text-amber-900 text-xs font-bold px-3 py-1 rounded-full mb-3 uppercase tracking-wider">Featured Story</span>
              <h1 className="font-[family-name:var(--font-display)] text-3xl md:text-5xl font-bold text-white leading-tight max-w-2xl mb-3">{featured.title}</h1>
              <p className="text-stone-200 max-w-xl text-base mb-5">{featured.excerpt}</p>
              <Link href={`/blog/${featured.slug}`} className="inline-flex items-center gap-2 bg-white text-stone-800 px-5 py-2.5 rounded-full font-semibold text-sm hover:bg-amber-400 hover:text-white transition-colors">Read Story →</Link>
            </div>
          </>
        ) : (
          <>
            <img src="https://images.unsplash.com/photo-1488646953014-85cb44e25828?w=1200&q=80" alt="Travel" className="absolute inset-0 w-full h-full object-cover" />
            <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/30 to-transparent" />
            <div className="relative z-10 max-w-6xl mx-auto px-4 pb-12 w-full">
              <h1 className="font-[family-name:var(--font-display)] text-3xl md:text-5xl font-bold text-white leading-tight max-w-2xl mb-3">Miles & Memories</h1>
              <p className="text-stone-200 max-w-xl text-base mb-5">The world is where I breathe.</p>
              <Link href="/admin/new" className="inline-flex items-center gap-2 bg-amber-400 text-white px-5 py-2.5 rounded-full font-semibold text-sm hover:bg-amber-500 transition-colors">Write First Story →</Link>
            </div>
          </>
        )}
      </section>
      ...
      <section className="max-w-6xl mx-auto px-4 py-16">
        ...
        {posts.length === 0 ? (
          <div className="text-center py-20 text-stone-400">
            ...
            <p className="text-lg">No posts yet. <Link href="/admin/new" className="text-amber-600 font-semibold">Write your first story, Caroline!</Link></p>
          </div>
        ) : (
        ...
        <div className="max-w-6xl mx-auto px-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              ...
              <Link key={cat.name} href={cat.name === 'All Posts' ? '/blog' : `/category/${cat.name.toLowerCase().replace(' & ', '-').replace(' ', '-')}`}
      ...
      <section className="max-w-6xl mx-auto px-4 py-16">
        <div className="bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-100 rounded-3xl p-8 md:p-12 flex flex-col md:flex-row items-center gap-8">
          <div className="flex-shrink-0 w-24 h-24 rounded-full bg-amber-200 flex items-center justify-center text-4xl">🌏</div>
          <div>
            ...
            <p className="text-stone-600 leading-relaxed">Born and raised in Mettur Dam, Tamil Nadu, India — now living in the USA since 2003. Mom of two, working full time, and always planning the next trip. I travel on a budget, take local buses and trains, stay in Airbnbs, and squeeze every hidden gem out of every destination I visit. Travel is where I forget the routine and find myself again.</p>
