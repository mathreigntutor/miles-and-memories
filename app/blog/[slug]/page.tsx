...
export default async function PostPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <article className="max-w-3xl mx-auto px-4 py-12">
      ...
      <h1 className="font-[family-name:var(--font-display)] text-3xl md:text-4xl font-bold text-stone-900 leading-tight mb-4">
        {post.title}
      </h1>
      ...
      <div className="mt-12 pt-8 border-t border-stone-100 flex gap-3">
        <Link href={`/admin/edit/${post.id}`} className="px-4 py-2 text-sm bg-stone-100 hover:bg-stone-200 text-stone-700 rounded-lg transition-colors">
          Edit Post
        </Link>
      </div>
    </article>
